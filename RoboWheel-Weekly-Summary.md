...
## D2 Keypoint IK（2026-04-14 晚）

替换 D1 的 Euler 分解映射为 **MANO keypoint IK**：

- `compute_mano_keypoints.py`：smplx MANO FK，每手 21 关键点（16 关节 + 5 指尖顶点）
- `mano_keypoint_ik.py`：MuJoCo per-finger FK + scipy `least_squares`
- 关键洞察：MANO canonical template ≡ Spider palm body frame（rest-pose 对齐 <1mm），IK 用 identity basis；世界系 wrist 的 R_M2S 不用于 keypoint 变换

效果：IK 残差 ~1-3cm；f60 握姿 `MCP=22°/PIP=42°/DIP=21°` 自然分布（D1 靠 CURL_GAIN 一刀切）。
